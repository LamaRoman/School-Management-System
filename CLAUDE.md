...
- `services/pdf.service.ts` — renders report card HTML and drives Puppeteer to produce the PDF. Two independent templates: marks-based and credit-grade (`buildCreditGradeReportCardHtml`), selected by `ReportCardSettings.gradingStyle`. Deliberately not shared code — see comments in that file for why.
...
- **Grading style** (`ReportCardSettings.gradingStyle`) — one report card design for the whole school (not picked per grade — that was the design until 2026-08-17, moved here so a school prints one consistent layout), set on the admin Report Card Settings page:
